...
testfile = "/mnt/Data/CloudStation/D-02/180213/D-02_180213_105759.vital"
devlist = [["DI-155", "VOLT"], ["DI-155", "ART1"], ["DI-155", "ECG"], ["Vigilance", "SV"]]
...
class vital_reader(object):
    ...
    def read_number_datetime(self, typename, trackname):
        ...
        for i in range(len(dt)):
            new_dt.append(datetime.datetime.fromtimestamp(dt))
        return dt, number
    ...
    def read_packets(self, track_list=[], skip_wave=False):
        i = 0
        try:
            with gzip.open(self.filename, "rb") as vital_file:
                while True:
...
#                        print ("Packet with 0 length was found. Ignoring.")
                        ...
                        if type == 0:  # SAVE_TRKINFO
                            ...
                            self.track[tid].name = packet_data.read(
                                int.from_bytes(packet_data.read(4), byteorder="little", signed=False)).decode('utf-8')
            ...
            if self.track[itrack].did != 0:
                ...
                if self.track[itrack].rec_type == 2:
                    ...
                    for i in range(len(self.track[itrack].v_number)):
                        if self.track[itrack].v_number[i] >= self.track[itrack].minval and self.track[itrack].v_number[i] <= self.track[itrack].maxval:
                            valid = valid + 1
                    ...
                    for i in range(len(self.track[itrack].v_wave)):
                        if self.track[itrack].v_wave[i] >= self.track[itrack].minval and self.track[itrack].v_wave[i] <= self.track[itrack].maxval:
                            valid = valid + 1
                    result_track = [device.devname, device.port, self.track[itrack].name, recoding_type, valid,
